...
const OfficerDrafts = ({ user, onViewDraft }) => {
  ...
  // ─── Delete draft ──────────────────────────────────────────────
  const handleDelete = async (draftUuid) => {
    if (!window.confirm("Are you sure you want to delete this draft?")) return;
    setActionLoading(draftUuid);
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${API_BASE}/client/draft/${draftUuid}?userId=${user?.id || ""}`,
        {
          method: "DELETE",
          headers: {
            Authorization: token ? `Bearer ${token}` : "",
      ...
      setDrafts((prev) => prev.filter((d) => d.draftUuid !== draftUuid));
    } catch (err) {
          ...
          >
            ...
            <div style={{ display: "flex", gap: "12px", marginTop: "12px", borderTop: "1px solid #f1f5f9", paddingTop: "12px" }}>
              ...
              {draft.status !== "submitted" && (
                <button
                  onClick={() => handleDelete(draft.draftUuid)}
