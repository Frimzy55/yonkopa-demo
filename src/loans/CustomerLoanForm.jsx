...
const CustomerLoanForm = ({ user, handleReset }) => {
    ...
    const fetchKyc = async () => {
      try {
        ...
        if (data.success && data.data) {
          ...
          // Extract fields – try multiple possible column names
  ...
  // ============================================================
  // The rest of the component (validation, steps, submit) is unchanged
  // ============================================================
  ...
  const nextStep = (e) => {
    ...
    if (isValid) {
      ...
      const allFields = {};
      Object.keys(formData).forEach(key => { allFields[key] = true; });
  ...
  const renderStep = () => {
    const stepProps = { formData, handleInputChange, handleFieldBlur, errors, touchedFields };
    switch (currentStep) {
  ...
  const getErrorSummary = () => {
    const summary = { applicant: [], loan: [], guarantor: [], momo: [] };
